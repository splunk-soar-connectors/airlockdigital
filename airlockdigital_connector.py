...
class AirlockDigitalConnector(BaseConnector):
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            try:
                ...
                return RetVal(
                    action_result.set_status(phantom.APP_ERROR, f"Unable to parse JSON response. {self._get_error_message_from_exception(e)}"),
                    None,
                )
    ...
    def _handle_disallow_hash(self, param):
        ...
        if applicationid == "" or None:
            ...
            request_json = {"hashes": [hash_param], "applicationid": applicationid}
    ...
    def _handle_block_hash(self, param):
        ...
        request_json = {"hashes": [hash_param], "blocklistid": param["blocklistid"]}

        url_req.append({"url": url, "header_var": self._header_var, "request_type": "blocklist"})

        # Make the request for each
        self.save_progress(f"Sending hash value to {AIRLOCK_HASH_BLOCKLIST_ADD_ENDPOINT}")
        ret_val, response = self._make_rest_call(url, action_result, json=request_json, headers=self._header_var, method="post")
    ...
    def _handle_allow_hash(self, param):
        ...
        # Required values can be accessed directly
        request_json = {"hashes": [{"path": param["path"], "sha256": hash_param}]}
        ...
        # Required values can be accessed directly
        request_json = {"hashes": [hash_param], "applicationid": param["applicationid"]}
    ...
    def _handle_list_identifiers(self, param):
        ...
        # If policy type is group
            ...
            return action_result.set_status(
                phantom.APP_ERROR, "Invalid policy type, please provide policy type either application, baseline, group, or blocklist"
            )

        # Make the request
        self.save_progress(f"Making request to URL: {url} with request type of {policy_type}.")
        ...
        if len(param_var.keys()) >= 1:
            if param_var["hostname"] != "all":
                ...
                ret_val, response = self._make_rest_call(
                    AIRLOCK_AGENT_FIND_ENDPOINT, action_result, json=param_var, headers=self._header_var, method="post"
                )
            else:
    ...
    def _handle_otp_revoke(self, param):
        ...
        param_var = {"otpid": otpid}
        # make rest call
        ret_val, response = self._make_rest_call(
            AIRLOCK_OTP_REVOKE_ENDPOINT, action_result, params=param_var, headers=self._header_var, method="post"
        )
    ...
    def _handle_otp_retrieve(self, param):
        ...
        param_var = {"agentid": agentid, "purpose": purpose, "duration": duration}

        # make rest call
        ret_val, response = self._make_rest_call(
            AIRLOCK_OTP_RETRIEVE_ENDPOINT, action_result, params=param_var, headers=self._header_var, method="post"
        )
    ...
    def initialize(self):
        ...
        self._header_var = {"X-APIKey": self._api_key}
